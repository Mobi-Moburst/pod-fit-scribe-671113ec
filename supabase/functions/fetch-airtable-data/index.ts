...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Use a per-connection token only when present; otherwise use the global secret.
    // Trim to avoid copy/paste whitespace causing Airtable auth failures.
    const perConnectionToken = typeof connection.personal_access_token === 'string'
      ? connection.personal_access_token.trim()
      : '';
    const globalToken = Deno.env.get('AIRTABLE_PAT')?.trim();
    const accessToken = perConnectionToken || globalToken;
