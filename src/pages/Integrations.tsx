...
export default function Integrations() {
  ...
  const loadMyConnection = async () => {
    ...
    if (error) {
      ...
      setMyConnection(data as FirefliesConnection | null);
    }
  ...
  const handleSyncNow = async () => {
    ...
    try {
      ...
      toast({
        title: "Sync complete",
        description: `Imported ${result?.imported ?? 0} new meeting(s).`,
      });
      await loadMyConnection();
      if (isAdmin) loadAdminData();
    } catch (err: any) {
  ...
  // Build admin overview rows: every user joined to fireflies connection
  const adminRows = allUsers.map((u) => {
    const conn = allConnections.find((c) => c.user_id === u.id);
    return { user: u, conn };
  });
      ...
      <main className="container mx-auto py-8 px-4 space-y-8 relative z-10">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Connect your personal Fireflies account so your meeting notes flow into the Command
              Center. Only your meetings are pulled — your key, your data.
            </CardDescription>
          </CardHeader>
          <CardContent>
            {isLoadingMine ? (
                  ...
                  <AlertDialog>
                    ...
                    <AlertDialogContent>
                      <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                          Your API key will be removed. Already-synced meeting notes stay put. You
        ...
        <Dialog open={showConnectDialog} onOpenChange={setShowConnectDialog}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Paste your personal Fireflies API key. We'll validate it and start pulling the last
                30 days of your meetings.
              </DialogDescription>
            </DialogHeader>
            <form onSubmit={handleConnect} className="space-y-4">
              ...
              <DialogFooter>
                <Button type="button" variant="ghost" onClick={() => setShowConnectDialog(false)}>
                  Cancel
                </Button>
